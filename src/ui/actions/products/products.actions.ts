...
class ProductsActions extends CommonActions {
  ...
  async getCreatedProductRow(productName: string) {
    return {
      ...
      manufacturer: await ProductsPage.getText(
        ProductsPage['Manufacturer by table row'](productName),
      ),
    };
  }
  ...
  async getProductToastText(text: ProductToastMessage, name?: string) {
    ...
    return (
      toastMessage(name) ||
      (text === 'already exist' && !name ? `Name was not provided` : 'Text was not provided')
    );
  }
}
